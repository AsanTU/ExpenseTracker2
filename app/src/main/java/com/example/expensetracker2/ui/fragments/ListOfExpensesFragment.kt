...
class ListOfExpensesFragment : Fragment() {
    ...
    private var selectedDate: LocalDate = LocalDate.now() // Default to today
...
//        setupSwipeToDeleteAndEdit(binding.expenseRv, adapter)
        ...
        // Display the default date in the filter text
        updateDateDisplay()

        // Fetch expenses for the default date (today)
        fetchExpenses()

        // Handle date picker selection
        binding.filterByDateText.setOnClickListener {
            showDatePicker()
        }

        binding.addExpenseBtn.setOnClickListener {
            findNavController().navigate(R.id.action_listOfExpensesFragment_to_addExpensesFragment)
    ...
    private fun fetchExpenses() {
        ...
        call.enqueue(object : Callback<ExpenseGetResponse> {
            override fun onResponse(
                call: Call<ExpenseGetResponse>,
                response: Response<ExpenseGetResponse>
            ) {
                if (response.isSuccessful) {
                    response.body()?.let { expenseGetResponse ->
                        if (expenseGetResponse.success) {
                            val fetchedExpenses = expenseGetResponse.expenses
                            adapter.notifyItemRangeRemoved(0, expensesList.size)
                            expensesList.clear()
                            expensesList.addAll(fetchedExpenses)
                            adapter.notifyItemRangeInserted(0, fetchedExpenses.size)
                        } else {
    ...
    private fun showDatePicker() {
        ...
        val datePickerDialog = DatePickerDialog(
            ...
            { _, selectedYear, selectedMonth, selectedDay ->
                ...
                updateDateDisplay()
                fetchExpenses() // Fetch expenses for the newly selected date
            },
            year, month, day
        )
